...
# ──────────────────────────────────────────────
# Phase 6 - Developer Communities Views
# ──────────────────────────────────────────────
from django.utils.text import slugify
from django.contrib import messages
from .models import Community, CommunityPost

def get_or_create_default_communities():
    default_comms = [
        ('Python Developers', 'python', 'General Python discussion, web development, data science, and scripting.', 'fab fa-python'),
        ('Web Development', 'web-dev', 'HTML, CSS, JavaScript, React, Django, Node, and everything web.', 'fas fa-globe'),
        ('Flutter & Mobile', 'flutter', 'Cross-platform app development using Flutter, Dart, Swift, and Kotlin.', 'fas fa-mobile-alt'),
        ('Java & Enterprise', 'java', 'Java, Spring Boot, enterprise architectures, and JVM languages.', 'fab fa-java'),
        ('UI/UX Design', 'ui-ux', 'Design patterns, user research, wireframing, and creating beautiful interfaces.', 'fas fa-palette'),
        ('AI & Machine Learning', 'ai-ml', 'Deep learning, neural networks, natural language processing, and generative AI.', 'fas fa-brain'),
    ]
    communities = []
    for name, slug, desc, icon in default_comms:
...
def communities_list_view(request):
    communities = get_or_create_default_communities()
    for c in communities:
        c.post_count = c.posts.count()
    return render(request, 'communities_list.html', {'communities': communities})

@login_required
def community_detail_view(request, slug):
    get_or_create_default_communities()
    community = get_object_or_404(Community, slug=slug)
    posts = community.posts.select_related('author', 'author__profile').order_by('-created_at')
    recommended = Community.objects.exclude(id=community.id)[:4]
...
def create_community_post_view(request, slug):
    ...
    messages.success(request, "Posted successfully to the community!")
    return redirect('community_detail', slug=slug)


@login_required
def premium_settings_view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'subscribe':
            if not premium:
                premium = PremiumUser.objects.create(user=request.user, is_active=True, badge_style='gold_star')
            else:
                premium.is_active = True
                premium.save()
            messages.success(request, "Welcome to Aetheria Premium! Your premium status is now active.")
        elif action == 'cancel':
            if premium:
                premium.is_active = False
                premium.save()
            messages.info(request, "Your Aetheria Premium subscription has been cancelled.")
        elif action == 'update_badge':
            badge_style = request.POST.get('badge_style', 'gold_star')
            if premium and premium.is_active:
                premium.badge_style = badge_style
                premium.save()
                messages.success(request, f"Your premium badge style has been updated to {badge_style.replace('_', ' ').title()}.")
        return redirect('premium_settings')

    context = {
        'premium': premium,
    }
    return render(request, 'premium_settings.html', context)
